// MARD 291 色完整色板
// 数据来源：https://www.pixel-beads.com/zh/mard-bead-color-chart（2026-08 抓取）
// 组织方式：按系列字母分组（A–H, M, P, Q, R, T, Y, ZG 共 15 个系列）
// 说明：官方未提供中文色名，name 字段用"X系列·序号"占位便于识别。
...
const RAW = [
  ...
  // T 系列（1 色）—— 已移除。T1 与 H2 均为纯白，为让所有纯白统一映射到 H2，
  // 此处不再收录 T1。原数据：['T1', '#FFFFFF']
...
// 展开为标准 palette 结构。name 无官方值，用 "系列·序号" 占位。
export const MARD_291 = RAW.map(([code, hex]) => {
  const m = /^([A-Z]+)(\d+)$/.exec(code)
  const series = m ? m[1] : ''
  const num = m ? m[2] : ''
  return {
    code,
    name: series ? `${series}系列·${num}` : code,
    hex,
